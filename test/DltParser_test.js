...
describe('Encoder test', () => {
    it('should encode correct from cmd to buf', () => {
        ...
        return expect(dltParser.cmdToBuf(cmd)).to.eventually.eql(encodedData);
    });

    it('should reject if cmd of wrong format', () => {
        const cmd =undefined;
        return expect(dltParser.cmdToBuf(cmd)).to.be.rejected;
    });
});
...
describe('Decoder test', () => {
    it('should decode correct from buf to cmd', () => {
        ...
        return expect(dltParser.bufToCmd(buf)).to.eventually.eql(cmd);
    });

    it('should reject if buf of wrong format', () => {
        const buf =undefined;
        return expect(dltParser.bufToCmd(buf)).to.be.rejected;
    });
});